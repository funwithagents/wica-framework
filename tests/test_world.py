...
def identity_serialize(value, previous_value):
    return str(value)
...
def test_update_type_mismatch_raises_and_leaves_value_untouched():
    ...
    assert '<entry key="temp" id="2">' in world.render_entry(world.get_entry("temp"))
...
def test_id_starts_at_one_on_register_and_increments_on_every_update():
    ...
    assert '<entry key="temp" id="1">' in world.render_entry(world.get_entry("temp"))

    world.update("temp", "a")
    assert '<entry key="temp" id="2">' in world.render_entry(world.get_entry("temp"))

    world.update("temp", "b")
    assert '<entry key="temp" id="3">' in world.render_entry(world.get_entry("temp"))
...
def test_clearing_advances_id_like_any_other_update():
    ...
    assert '<entry key="temp" id="4">' in world.render_entry(world.get_entry("temp"))
...
def test_serialize_fn_receives_correct_previous_value():
    ...
    def recording_serialize(value, previous_value):
...
def test_render_entry_format():
    world = World()
    world.register("user_profile", str, serialize_fn=lambda v, p: "Jane is logged in.")
    world.update("user_profile", "irrelevant raw value")

    rendered = world.render_entry(world.get_entry("user_profile"))
    match = re.match(
...
def test_render_entry_ignores_include_in_prompt():
    ...
    assert "secret" in world.render_entry(world.get_entry("hidden"))
...
def test_render_full_prompt_omits_excluded_entries():
    ...
    prompt = world.render_full_prompt()
...
def test_render_full_prompt_block_matches_render_entry():
    ...
    assert world.render_full_prompt() == world.render_entry(world.get_entry("solo"))
