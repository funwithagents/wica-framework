...
| [conversation-demo.md](conversation-demo.md) | Conversation demo product: browser UI to talk to a simulated social robot — the three `wica.contrib.gradio` panels (transcript, World state, prompt history) driven through the demo's `display_entry` hook, plus its own Speaking panel; simulated sensor inputs and robot-action Commands; composed build → UI → wire → start | Implemented |
...
| [gradio-contrib.md](gradio-contrib.md) | Reusable Gradio observability components as an opt-in `wica[gradio]` contrib package: live World-state panel, prompt-history panel (`PromptLog`), conversation transcript (`TranscriptLog`, `display_entry` hook, live Command state), lifted from the demo; presenter + self-refreshing panel per surface; core stays UI-free | Implemented |
