...
1. **Output: explicit `speak` Command vs. free-text-as-speech.** v1 sends the model's complete free-text response to the output sink. A future streaming implementation could preserve that rule or introduce `speak(text)` for explicit control over spoken versus internal text.
...
- **`speak()` Command.** v1 uses free-text-as-speech. An explicit `speak(text)` Command remains a live alternative if visible text and spoken output need to diverge.
