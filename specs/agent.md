...
**`AgentConfig` is unresolved data; construction is where its references resolve.** A loaded `AgentConfig` mirrors the JSON and holds `api_key`/`api_key_env` and `system_prompt`/`system_prompt_file` verbatim — no env or file read has happened. The Agent-side construction path performs those reads at build: `build_chat_model` calls `resolve_api_key(config)` (env-var read, `MissingEnvError` if unset) and `Agent.from_config` calls `resolve_system_prompt(config)` (prompt-file read). Both resolvers are pure functions owned by [config.md](config.md); the Agent is just the consumer that invokes them — so an unset key env or unreadable prompt surfaces here, at `Agent.from_config`, not at config load.
...
This is still one adapter at the Agent's I/O boundary, not a leak upstream: the `huggingface-hub` branch is just one more provider-construction detail inside `Agent.from_config`. Everything above the Agent stays SDK-agnostic regardless of provider.

### Configurable system prompt

The Agent takes a system prompt from config, so the agent/robot's persona and behavior are customizable per deployment without code changes. The config may carry it inline (`system_prompt`) or as a file reference (`system_prompt_file`); `Agent.from_config` resolves the two to a single string via `resolve_system_prompt(config)` at build (see [config.md](config.md), "System prompt"), so `Agent.__init__` still receives a plain resolved `system_prompt: str` and the loop never sees the file indirection.

### The Agent owns the event loop; the World stays sync

The World is sync and thread-based (see [world.md](world.md), open question #3): `update()` and its callbacks can be driven from any thread (sensor/hardware/bookkeeping producers). The Agent needs an asyncio event loop for cancellable async Commands. Resolution:
...
The World is **not** given the loop (rejected alternative in world.md open question #3).
...
- **All window manipulation happens on the loop thread.** The window timer is opened, joined, flushed, and cancelled only on the Agent's single event loop — the same invariant that makes cancellation race-free (see "The Agent owns the event loop"). The World→loop bridge still just schedules `_handle_trigger` onto the loop; the window logic lives inside it.
- **`on_trigger` fires once per collected trigger.** Even though a coalesced burst runs a single step, the `on_trigger` instrumentation hook fires **once for each trigger** that joined the window, so a consumer like the demo still shows every input on the conversation's input side (see Instrumentation). The other per-step artifacts — the Observation record, `on_prompt`, and the model call — happen **once** for the coalesced step.
...
|---|---|---|
| `on_trigger` | `Callable[[WorldEntry], None]` | **once per trigger that a run-to-completion step observes**, with the World entry whose update caused it. A burst of *N* triggers coalesced into one step (see "Trigger coalescing") fires the hook *N* times but runs a single step; a dropped single-in-flight trigger fires nothing. Includes command-completion re-triggers; a consumer that only cares about external inputs filters out `agent:command:*` keys itself |
