...
- The Agent **auto-registers one WICA-native control Command, `cancel_command(call_id)`** (see [commands.md](commands.md), "`cancel_command`"), so the model can abort a Command it previously issued that is still `running`. It targets the command by its `call_id` — already on screen in the `<entry key="agent:command:<call_id>" …>` envelope, so no extra rendering — and reuses the Agent's existing loop-thread cancellation path (the same one `stop()` drives via `Agent.cancel_command`); an already-finished/unknown target is a harmless no-op. This is the v1-buildable sibling of the deferred `cancel_activity(task_id)`, which cancels an in-flight *LLM call* rather than a Command (see "Concurrency & interruption").
