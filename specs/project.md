...
- **The e2e tier is parametrized over one committed config per provider**, not a single reference provider. Each supported provider has its own checked-in config (`tests-e2e/e2e.<provider>.config.json` for `anthropic`/`openai`/`huggingface-hub`), wired together as `PROVIDER_CONFIGS` in `tests-e2e/support.py`, so **every e2e test runs once per config** — exercising WICA's real provider-branching construction (`build_chat_model`/`Agent.from_config`), `huggingface-hub`'s dedicated path included, rather than a single privileged provider. Each config uses `api_key_env` (see [config.md](config.md), "API key"), so a provider whose key env var is unset **skips itself** (never fails) via `MissingEnvError → pytest.skip`: you only exercise the providers you hold keys for, and contributors without credentials — and CI, unless deliberately configured — aren't broken by their absence. Filter to one provider with `-k <provider>` (the configs are named symmetrically, so the provider name matches its config-filename stem)
