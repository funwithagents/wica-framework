...
- **Agent-produced entries are not Inputs.** Command-execution entries (`agent:command:<call_id>` — see [commands.md](commands.md)), the Agent's own reaction state (a future `agent:reaction:<id>` entry — see [agent.md](agent.md), "Future improvements"), and entries the model sets via Commands (in the demo, `emotion` and `tracked_user`) are all written *by the Agent as a result of its own reasoning*. They're World state, but they're not perception coming in.
...
|---|---|
| `serialize_fn` | Turns the raw perception into `Content` for the prompt — `[TextPart("The user said: …")]` for speech, an `ImagePart` for a camera frame, an `AudioPart` (once it exists — see [content.md](content.md)) for a mic clip. This is where a sensor reading becomes something the model can read. Returning `[]` omits the Input from the prompt for that render (see [world.md](world.md), "Omitting an entry") — the natural way to keep a perception that hasn't happened yet (value `None` before the first reading, or after a TTL reset) out of the model's view, instead of narrating its absence |
