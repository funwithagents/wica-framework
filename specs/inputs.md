...
- **Agent-produced entries are not Inputs.** Command-execution entries (`agent:command:<call_id>` — see [commands.md](commands.md)), the Agent's own reaction state (`agent:reaction[...]` — see [agent.md](agent.md)), and entries the model sets via Commands (in the demo, `emotion` and `tracked_user`) are all written *by the Agent as a result of its own reasoning*. They're World state, but they're not perception coming in.
