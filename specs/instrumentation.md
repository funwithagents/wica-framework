...
- **Reaction history** ([gradio-contrib.md](gradio-contrib.md), "Component 2"): a `ReactionLog` presenter merges each reaction's prompt (`on_agent_prompt`) and its trace (`on_agent_reaction_ended`, matched by `reaction_id`) into one record, browsable through one dropdown with a "Prompt" tab and an "Instrumentation" tab — the natural place to *inspect one reaction's numbers on demand*, as opposed to watching a rolling aggregate (below).
...
- **A rolling metrics view** — current state (idle / coalescing / thinking / speaking), rolling last/median/p95 of the measures above *across many reactions* — is a different shape from the reaction history above (one reaction at a time, on demand) and remains the natural next consumer; its shape is an open question below.

## Open questions

Deferrals; none blocks the design above.

1. **A rolling/aggregate metrics view.** Presenter + panel like the others (`MetricsLog(wica)` + `metrics_panel`), or a strip inside the transcript? Distinct from the per-reaction "Instrumentation" tab in [gradio-contrib.md](gradio-contrib.md) ("Component 2"), which already lets a person inspect one reaction's numbers on demand — this item is about a rolling aggregate across many reactions. Wait for real traffic and look at real numbers first.
