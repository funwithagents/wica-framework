...
- On completion, the model continues via the **reconstruct path**: at the next step the Agent re-renders the earlier Command from its durable, provider-neutral `CommandRecord` (name/args/`call_id`) as the model's **native `tool_call`**, paired with a **`tool_result`** carrying the stringified outcome, and picks up from there. The native pair is reconstructed *only at render time*, inside the Agent's message adapter — the same place LangChain/provider blocks already live — never stored in history. *(Earlier design rendered the Command as a `"Calling walk_to…"` assistant **text** line instead; that backfired — the model imitated the prose and emitted command descriptions as free text instead of issuing real tool calls, so the reconstruct path now uses the native pair. Because the outcome rides on the `tool_result`, **any** `agent:command:<call_id>` World entry is **not** re-rendered into the observation (the skip is by key prefix, not by state — see [agent.md](agent.md), "Rendering to messages"), avoiding a duplicate.)*
...
Consequence for history ([agent.md](agent.md), "History record shape"): the durable record of a Command is this **neutral description** — a `CommandRecord` (name/args/`call_id`) plus its result — **not** raw provider `tool_use`/`tool_result` messages. Those provider-native blocks are **reconstructed from the neutral record at render time** (in the Agent's message adapter) so the model sees its past actions in the exact format it emits them, then discarded; they are never what's stored. History stays snapshots + neutral descriptions.
