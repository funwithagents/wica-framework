...
- **`Command` wraps a callable or an off-the-shelf tool and holds the backing `BaseTool` internally.** Its constructor accepts **either** a plain `Callable[..., Any]` **or** a LangChain `BaseTool`:
  - `Command(fn, *, name=None, description=None, triggers_on_completion=True)` — builds the backing tool from the function (name from `__name__`, description from the docstring; explicit `name`/`description` override). An undocumented function with no `description` fails loudly, the same `tool()` requirement WICA already inherits.
  - `Command(existing_tool, *, triggers_on_completion=True)` — wraps an off-the-shelf `BaseTool` unmodified. "Works unmodified" still holds: you *wrap*, you don't *modify* — it's just explicit now. `name`/`description` may not be overridden here (they are the tool's own), but `triggers_on_completion` may: it is a WICA registration option, not tool metadata.
- **`triggers_on_completion` (default `True`) — whether this Command's terminal completion wakes a reasoning step.** Exposed as a read-only property. It is the per-Command form of the single knob that used to exist only for the output Command (`_TRIGGER_ON_OUTPUT_COMMAND_COMPLETION`, now that Command's *default* — see "The output Command"), generalized because any Command whose outcome the model does not need to react to pays a wasted inference under the uniform re-trigger model: a fire-and-forget actuator (`set_led`), a bookkeeping write, or — the case that motivated it — a test harness's own `set_<input>` Commands on a tester agent, each of which would otherwise cost a full extra step ([_wica-tester-study.md](_wica-tester-study.md), §3.3). `False` only removes the *trigger*: the `agent:command:<call_id>` entry is still registered, still `include_in_prompt`, still goes `running` → terminal, and is still observed and retired by the next step that runs for any other reason — exactly the "completion whose trigger was dropped" path below. It is the World's `triggers_llm_call` for that entry, nothing more.
...
- **`Command` is where per-command options live.** `triggers_on_completion` (above) is the first; the deferred sync/async `mode` (see "Sync vs. async") and a custom result-rendering hook ([Open questions](#open-questions) #5) are natural `Command` fields when built — additive.
- **Names are unique and two are reserved.** `noop` and `cancel_command` belong to WICA. `Agent.register_command()` raises `ValueError` for either name and for any name already registered. The optional output Command's name is checked when it is set (`Agent.set_output_command`: a reserved name, or a name already registered as an application Command, raises `ValueError` and leaves the Agent unchanged — see [agent.md](agent.md), "Output wiring") and is then reserved too. WICA's own Commands are attached by a private idempotent path used by `Agent.start()`, so a restart re-attaches them without ever replacing an application Command.
...
The output Command runs through the **same uniform lifecycle** as any other Command below — `running` → terminal `agent:command:<call_id>` entry — with exactly one tunable difference: **whether its terminal completion re-triggers a step.** That is the `Command.triggers_on_completion` flag (see "The `Command` object"); for the output Command its *default* is the module-level constant `_TRIGGER_ON_OUTPUT_COMMAND_COMPLETION` (default **`True`**), applied when `set_output_command` is handed a bare callable — a `Command(fn, triggers_on_completion=…)` passed explicitly keeps its own value. The flag gates the `triggers_llm_call` the entry is registered with:
...
Shipping default is `True`; the constant exists so flipping the framework-wide default is a one-line change, not a refactor, and the per-Command flag lets one application choose differently without touching it. The choice is recorded, not hidden, precisely because the cost/behavior trade is real. `include_in_prompt` stays `True` regardless, so barge-in works either way.
...
- It **is** still a command the model issued, so the Agent's `on_command` instrumentation Event fires for it (`CommandIssued("noop", {}, call_id)` — a `call_id` with no World entry behind it) like for every other issued command — observability, not action. A consumer wanting only World actions filters `noop` (and the output Command) out by name; one wanting to *show* that the agent declined (as the conversation demo does) renders it. See [agent.md](agent.md), "Instrumentation".
...
- Register the command-execution entry (`agent:command:<call_id>`) with `triggers_llm_call=<the Command's triggers_on_completion>` (`True` for every Command by default — see "The `Command` object"; a name the model issued that matches no registered Command registers with `True` and then fails) **and** `trigger_condition_fn=lambda old, new: new.is_terminal()`. Start/progress updates land in the entry (visible to any reader) but **fail the condition → no trigger**; only **complete/failed/cancelled** triggers a new Agent step.
...
A response carrying several tool calls dispatches **all of them at once** — one running entry and one task each, created in the response's order, then run concurrently with no ordering or exclusivity guarantee between them (see [agent.md](agent.md), "Concurrency and long-running Commands in v1"); the runtime primer tells the model so, and that sequencing dependent actions means issuing the later one in a later step.
