...
- **Config-driven construction *is* the constructor: exactly one build path.** `Agent.__init__` takes the `AgentConfig` directly. `Wica.init` likewise takes the already-created `WicaConfig`; the `**kwargs` carry code-only wiring a config object should not express (event loop, output sink/output Command, `coalesce_window`). There is deliberately no path-taking convenience such as `Wica.from_json_file`: it would privilege one input adapter and hide the config object the caller often wants.
