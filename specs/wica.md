...
`Wica` **owns** its World and Agent; the public `wica.world`/`wica.agent` attributes are **borrowed references, valid only for the life of the `Wica`**. A consumer uses them while the `Wica` is alive and discards them with it — the same contract as a file object obtained from a context manager. After `wica.stop()`, the World is stopped and touching a mutating operation on a held reference raises `RuntimeError("World is not running")` (see [world.md](world.md), "Lifecycle") — fail-fast rather than a silent stale read. The stated rule: *`wica.world` is borrowed from Wica; after `wica.stop()` it is inert — get a fresh one from a new Wica.*
...
1. **Multiple `Wica` instances.** Dropping the World singleton (see [world.md](world.md)) makes several `Wica` instances *structurally* possible — each owns its own World/Agent, with no shared global. Whether multi-instance is a *supported, tested* configuration (e.g. two independent agents in one process) is not yet exercised; today's single-agent-per-World assumption ([agent.md](agent.md), open question #10) still frames the design. The facade doesn't prevent multiple instances; it just isn't validated for them yet.
