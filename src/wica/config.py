...
def _validate_api_key(data: dict[str, Any], *, block: str) -> tuple[str | None, str | None]:
    ...
    api_key_env = _require_str(data, "api_key_env", block=block) if has_env_ref else None
...
def resolve_system_prompt(config: AgentConfig) -> str:
    ...
    if config.system_prompt_file is not None:
        ...
        try:
            ...
            raise ConfigError(f"could not read system_prompt_file {prompt_path}: {exc}") from exc
    raise ConfigError("agent: no system prompt configured")  # unreachable given validation
